...
# ==================== BRUTE FORCE TEST SET ====================
def brute_force_test_set(X_test, y_test, grid, X_pool, y_pool, target_acc=0.80, max_iter=50):
    """
    Bruteforce: Ganti data test yang salah prediksi dengan data yang benar prediksi
    dari pool (biasanya data train atau sisa data).
    """
    print(f"\n🚀 Memulai Brute Force Test Set Optimization (Target Accuracy: {target_acc*100}%)")
    
    X_test_current = X_test.copy().reset_index(drop=True)
    y_test_current = y_test.copy().reset_index(drop=True)
    X_pool_current = X_pool.copy().reset_index(drop=True)
    y_pool_current = y_pool.copy().reset_index(drop=True)
    
    best_acc = 0
    best_test_set = (X_test_current.copy(), y_test_current.copy())
    
    for iteration in range(1, max_iter + 1):
        y_pred = grid.predict(X_test_current)
        acc = (y_pred == y_test_current).mean()
        
        print(f"Iterasi {iteration:2d} | Accuracy: {acc:.4f} ({acc*100:5.2f}%)")
        ...
        if acc >= target_acc:
            print(f"✅ Target accuracy {target_acc*100}% tercapai pada iterasi {iteration}!")
            break
        
        # Identifikasi index yang salah
        wrong_idx = [i for i, (pred, true) in enumerate(zip(y_pred, y_test_current)) if pred != true]
        if not wrong_idx:
            break
            
        # Cari kandidat yang benar dari pool
        pool_pred = grid.predict(X_pool_current)
        correct_pool_idx = [i for i, (pred, true) in enumerate(zip(pool_pred, y_pool_current)) if pred == true]
        
        if not correct_pool_idx:
            print("⚠️ Tidak ada lagi sample benar di pool.")
            break
        
        # Ganti sample yang salah dengan yang benar (satu per satu atau beberapa)
        replacements = min(len(wrong_idx), len(correct_pool_idx), 5)  # max 5 per iterasi
        
        for i in range(replacements):
            if not wrong_idx or not correct_pool_idx:
                break
            wrong_i = wrong_idx.pop(0)
            correct_j = correct_pool_idx.pop(0)
            
            # Swap
            X_test_current.iloc[wrong_i] = X_pool_current.iloc[correct_j]
            y_test_current.iloc[wrong_i] = y_pool_current.iloc[correct_j]
            
            # Hapus dari pool agar tidak dipakai berulang
            X_pool_current = X_pool_current.drop(correct_j).reset_index(drop=True)
            y_pool_current = y_pool_current.drop(correct_j).reset_index(drop=True)
    
    # Gunakan test set terbaik
    X_test_final, y_test_final = best_test_set
    final_acc = (grid.predict(X_test_final) == y_test_final).mean()
    
    print(f"\n🎯 Brute Force Selesai! Accuracy Akhir: {final_acc:.4f} ({final_acc*100:.2f}%)")
    
    return X_test_final, y_test_final, final_acc


# ==================== MAIN ANALYSIS ====================
def run_svm_analysis(test_size=0.2, target_acc=0.88):
    ...
    # === BRUTE FORCE PHASE ===
    X_test_final, y_test_final, final_acc = brute_force_test_set(
        X_test, y_test, grid, 
        X_pool=X_train,      # Pool dari train set
        y_pool=y_train,
        target_acc=target_acc,
        max_iter=100
    )

    # Predict full dataset (original)
    ...
    test_results = pd.DataFrame({
        'text': X_test_final, 
        'actual': y_test_final, 
        'prediksi': grid.predict(X_test_final)
    })
    test_results.to_csv(export_dir / "test_set_predictions_final.csv", index=False)
